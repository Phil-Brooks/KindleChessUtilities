...
module Pgn = 
    ...
    let ReadGames(fn:string) =
        let stream = new FileStream(fn, FileMode.Open)

        let rec nextGameRdr(sr) = 
                    ...
                    |InRes ->
                        ...
                        let ngm =
                            if bits.Length=1 then
                                let mte = GameEndEntry(cstr)
                                {gm with MoveText=mte::gm.MoveText}
                            else
                                let mte = GameEndEntry(bits.[0].Trim())
        ...
        let rec getGames gml = 
          if sr.EndOfStream then gml|>List.rev
          else
